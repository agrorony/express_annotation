...
def test_reliability_flag():
    """Test 5: Vogel et al. reliability constraint (d < 5 voxels)."""
    print("\n" + "=" * 60)
    ...
    print(f"  Unreliable bins (d < 5 voxels): {n_unreliable}")
    print(f"  Reliable bins (d >= 5 voxels): {n_reliable}")
    ...
    if len(unreliable_diameters) > 0:
        ...
        if unreliable_diameters.max() < 5.0:
            ...
            print("  Status: ✗ FAIL (some unreliable bins >= 5 voxels)")
    else:
