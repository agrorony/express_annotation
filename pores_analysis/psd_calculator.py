...
Scientific Constraints (Vogel et al., 2010):
    1. Reliability Flag: Diameters < 5 voxels are unreliable (flagged)
...
# Scientific Constants (Vogel et al., 2010)
MIN_RELIABLE_DIAMETER_VOXELS = 5  # Hard constraint from paper
...
def mask_border_voxels(
    ...
    Args:
        ...
        border_width: Width of border region to mask (default 1 voxel)
...
def compute_psd_from_opening_map(
    ...
    Pipeline:
        ...
        5. Apply reliability flags (d < 5 voxels)
    ...
    Returns:
        Dictionary with:
            ...
            - 'reliability_flag': Boolean array (True = reliable, d >= 5 voxels)
    ...
    volume_counts, _ = np.histogram(pore_diameters, bins=bin_edges)
    ...
    # Convert bin centers to voxel units for comparison
    voxel_size_avg = np.mean(voxel_spacing)  # Average voxel size
    bin_centers_voxels = bin_centers / voxel_size_avg
    reliability_flag = bin_centers_voxels >= MIN_RELIABLE_DIAMETER_VOXELS
    
    return {
        'bin_centers_px': bin_centers / voxel_size_avg,
...
def compute_psd(
    ...
    if use_chunking:
        ...
        def edt_func(block):
            return compute_edt(block, voxel_spacing, use_gpu=use_gpu)
        ...
        # Monolithic EDT
        edt_map = compute_edt(binary_volume, voxel_spacing, use_gpu=use_gpu)
    
    print(f"  EDT complete. Max distance: {edt_map.max():.2f} μm")
    ...
    print(f"  Opening complete. Max radius: {opening_map.max():.2f} μm")
